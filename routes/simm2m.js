...
router.post('/', canEdit('m2m'), (req, res) => {
  const { iccid, phone_no, operator, status, company, plate_no, vehicle_type, notes, package_id } = req.body;
  ...
  const result = db.prepare(`
    INSERT INTO sim_m2m (iccid, phone_no, operator, status, company, plate_no, vehicle_type, notes, package_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(iccid || null, cleanPhone, operator, status || 'active',
         company || null, plate_no || null, vehicle_type || null, notes || null, package_id || null);
...
router.put('/:id', canEdit('m2m'), (req, res) => {
  ...
  if (cleanPhone) {
    const exists = db.prepare(`
      ...
      SELECT 1 FROM sim_voice WHERE phone_no = ?
      LIMIT 1
    `).get(cleanPhone, req.params.id, cleanPhone, cleanPhone);
    if (exists) return res.status(400).json({ message: 'Bu telefon numarası zaten kayıtlı.' });
  }
  ...
  const result = db.prepare(`
    UPDATE sim_m2m
    SET iccid=?, phone_no=?, operator=?, status=?, company=?, plate_no=?, vehicle_type=?, notes=?, package_id=?, updated_at=CURRENT_TIMESTAMP
    WHERE id=?
  `).run(iccid || null, cleanPhone, operator, status, company || null,
...
router.post('/bulk-update', canEdit('m2m'), (req, res) => {
  ...
  const allowedFields = ['operator', 'status', 'company', 'vehicle_type', 'notes', 'package_id'];
